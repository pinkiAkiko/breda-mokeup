...
- `Apply for Clearances.dc.html` — departmental clearance basket (7 depts). Each service = checkbox row; expanding a checked row shows supporting-doc + fee-challan upload **except the BSPTCL rows**, where the statutory form replaces it (FORMAT-2 Section F + Enclosure 1 already carry the fee) — those rows are gated by form completion alone, the rest by challan/receipt. **BSPTCL statutory forms live in-row, as tabs** (built Sept 2026, from `uploads/BSPTCL, STU Clearance-pages.pdf`):
  ...
  - **Connection Agreement — Intra-State Transmission Licensee → FORMAT-4 (Licensee)** (`form:'F4L'`, added Sept 2026, kept as its own row/kind rather than folded into FORMAT-4's `isLicensee()` branch — client asked for the two-page STU sample "FORMAT-4 applicable in case of grant of connectivity to the Intra-State transmission licensee" to be its own clearance). Tabs: Applicability · Licensee (A) · Maps (B, Sch. I–III) · Connection (C) · Communication (D) · Certification (E) — no generating-plant schedules, no applicant-status branching. `f4lTabs()`/`f4lGroups()`, dispatched in `tabsFor`/`groupsForKind`/`signTabFor` (sign tab `'E'`) alongside the F2/F4/WCR/WR kinds.
  - FORMAT-2 and FORMAT-4 (not the Licensee variant) branch on **applicant status** (generator / captive generator / distribution licensee / captive user / OA consumer): Section C is dropped for a distribution licensee; Section D has four statutory branches; FORMAT-4 also branches applicant ↔ intra-State transmission licensee internally via `isLicensee()`. All FORMAT-4 kinds derive the **two-month acceptance deadline** from the FORMAT-3 offer date.
